...
class Config:
    def __init__(self, config_file_path):
        with open(config_file_path, 'r') as file:
            self.data_collection = yaml.safe_load(file)

# Load config from YAML file
config_file_path = 'config/data_collection/rover_dara.yaml'
config = Config(config_file_path).data_collection
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print('Usage: ')
        exit('$ rosrun data_collection data_collections.py your_data_id')
