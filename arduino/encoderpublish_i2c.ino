...
void setup() {
  ...
  Wire.begin(I2C_ADDRESS);  // Initialize I2C with slave address
...
// I2C event handler to send data on request
void requestEvent() {
  // Send the throttle wheel position (input)
  Wire.write((int)(input) >> 8);  // Send high byte
  Wire.write((int)(input) & 0xFF);  // Send low byte
  
  // Send the steer wheel position (mapped value)
  Wire.write((int)(steer_wheel_msg.data * 100) >> 8);  // Multiply by 100 and send high byte
  Wire.write((int)(steer_wheel_msg.data * 100) & 0xFF);  // Send low byte
}
